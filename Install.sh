...
sudo apt-get install -y "${APT_PACKAGES[@]}" "${APT_OPTIONAL[@]}" dconf-cli gnome-tweaks gnome-shell-extension-manager uuid-runtime xclip

# -----------------------------
# 1️⃣ Extra: Install Zsh plugins if missing
# -----------------------------
ZSH_SHARE="/usr/share/"
...
if [ ! -d "$NVIM_CONFIG" ] || [ -z "$(ls -A "$NVIM_CONFIG")" ]; then
  ...
  if ! nvim --headless "+Lazy! sync" +qa; then
      echo "⚠️ LazyVim sync failed. You may need to run 'nvim' manually to complete setup."
  fi
fi
...
echo "Choose a GNOME Terminal theme by number (1, 2, or 3):"
select choice in Dracula SolarizedDark GruvboxDark; do
    if [[ -n "$choice" ]]; then
        ...
        } >/dev/null 2>&1 || echo "⚠️ Failed to apply terminal theme. You may need to set it manually."
